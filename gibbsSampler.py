...
def gibbsSampler(text, k, t, N):
    ...
    for j in range(N):
        ...
        tempMotif = localMotifs
        del tempMotif[i]
        profileMatrix = buildProfile(tempMotif, k)
        tempMotif.append(probableKmer(text[i], k, profileMatrix))

        localScore = scoringFunction(tempMotif, k)
        if bestScore > localScore:
            bestMotifs = tempMotif
            bestScore = localScore
        localMotifs = tempMotif
